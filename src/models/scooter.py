...
def update_scooter(scooter_id, fields: dict):
    ...
    try:
        ...
        if not all(field_name in FIELD_COLUMN_MAPPING for field_name in fields.keys()):
            ...
            # Log as suspicious via centralized encrypted logger
            log_instance.addlog(
                username="system",
                action="Invalid scooter update fields",
                details=f"Invalid fields: {invalid_fields}; scooter_id={scooter_id}; at {datetime.now().isoformat()}",
                suspicious=True,
            )
            return False
